...
if ~isfield(options_parsed,'frames')
    ...
    if img.frames > 1
        warning(['img_FindPeaks(): image contains multiple frames and ',...
...
if img.frames > 1
    ...
    for fr = frames
        ...
        [img_temp, p_temp_vol, p_temp] = img_temp.img_FindPeaks(mindim, maxdim, val, t, projection_raw, options_single_frame, verbose);
...
if strcmpi(img.imageformat, 'CIFTI-2')
    ...
    vol_sections = img.img_ExtractCIFTIVolume();
    
    % --- Find peaks for volume components
    if verbose, fprintf('\nMAIN FIND PEAKS---> finding peaks for volume components'); end
    [vol_roi vol_peak] = vol_sections.img_FindPeaksVolume(minsize, maxsize, val, t, options, verbose_pass);
    
    % --- Embed volume components
    if verbose, fprintf('\nMAIN FIND PEAKS---> embedding volume components'); end
    roi = img.img_EmbedCIFTIVolume(vol_roi);
    ...
    for i=1:1:numel(img.cifti.shortnames)
        if strcmp(cifti.(lower(img.cifti.shortnames{i})).type,'Surface')
            [roi peak.(lower(img.cifti.shortnames{i}))] = roi.img_FindPeaksSurface(lower(img.cifti.shortnames{i}),...
...
elseif strcmpi(img.imageformat, 'NIFTI')
    if verbose, fprintf('\nMAIN FIND PEAKS---> finding peaks for volume components'); end
    [roi vol_peak] = img.img_FindPeaksVolume(minsize, maxsize, val, t, options, verbose_pass);
    peak = [];
end
