    ...
    for i=1:length(roicodes1)
        for j=1:length(roicodes1{i})
            if ~any(roi1.data == roicodes1{i}(j))
                switch lower(check)
                    case 'warning'
                        warning('\nimg_ReadROI: Code [%d] does not exist in %s!\n',roicodes1{i}(j),roif1);
                    case 'error'
...
for n = 1:nroi
    ...
    if ((length(roicodes1{n}) == 0 || isempty(roi1)) & (~isempty(roi2)))
        rmask = roi2.img_ROIMask(roicodes2{n});
    elseif ((length(roicodes2{n}) == 0 || isempty(roi2)) & (~isempty(roi1)))
        rmask = roi1.img_ROIMask(roicodes1{n});
    elseif ((~isempty(roi2)) & (~isempty(roi1)));
        rmask = roi1.img_ROIMask(roicodes1{n}) & roi2.img_ROIMask(roicodes2{n});
    else
        rmask = [];
    end
