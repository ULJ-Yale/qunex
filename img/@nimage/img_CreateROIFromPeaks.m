...
if strcmpi(img.imageformat, 'CIFTI-2')
    surfaceImage = img;
    img = surfaceImage.img_ExtractCIFTIVolume();
    embedBack = true;
end

% --- convert xyz to ijk
ijk = img.img_GetIJK([X, Y, Z]);
...
if embedBack
    surfaceImage = surfaceImage.img_EmbedCIFTIVolume(img);
    img = surfaceImage;
end
