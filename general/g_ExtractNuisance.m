function [nuisance hdr] = g_ExtractNuisance(img, fsimg, bmimg, target, ntarget, wbmask, sbjroi, nroi, nimg, verbose);
...
if ~strcmp(ntarget, 'none')
    ...
    img = img ./ 2800 % max(max(max(img))); --- Change due to high values in embedded data!
    %img = img * 0.7;
    ...
    imwrite(img, fullfile(ntarget, [fname '_nuisance.png']), 'png');
end
