function data = g_Smooth3D(data, fwhm)
...
voxels = 48*64*48;
data = reshape(data, voxels, []);
frames = size(data,2);
data = reshape(data, [48, 64, 48, frames]);

fprintf('Smoothing frame    ');
for n = 1:frames
	fprintf('\b\b\b\b%4d',n);
	data(:,:,:,n) = smoothvolume(data(:,:,:,n), ksd);
end
fprintf('\b\b\b\b ... finished');

data = reshape(data, insize);
