...
show_usage() {
				...
				echo "ap -<command>   OR   ap ?<command>   OR   ap <command>"
				echo ""
				echo "---------------------------------------------------------------------"
				...
				echo ""
				cyaneho "Additional neuroimaging utilities for preprocessing and analyses (gmri package)"
				cyaneho "-------------------------------------------------------------------------------"
				echo ""
				...
				echo ""
				echo "--> Example to pass command: 				ap <command> [options]"
				echo "--> Example to request help for command : ap ?<command>"
				echo ""
				cyaneho "Full help output for 'gmri' utilities is provided below:"
				cyaneho "---------------------------------------------------------"
				echo "`gmri`"
				echo "`gmri -l`"
				echo ""
...
qcpreproc() {
	...
	if [ "$Modality" == "BOLD" ]; then
		...
		if [ "$Cluster" == 2 ]; then
			...
			cd ${LogFolder}
			gmri schedule command="${LogFolder}/${CASE}_ComQUEUE.sh" settings="${Scheduler}" output="stdout:${LogFolder}/qcpreproc.output.log|stderr:${LogFolder}/qcpreproc.error.log"  workdir="${LogFolder}" 
			echo ""
			echo "---------------------------------------------------------------------------------"
			echo "Data successfully submitted" 
			#echo "Scheduler Options: $SchedulerOptions"
