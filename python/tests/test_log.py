...
def test_the_version_is_read_from_the_tree_the_code_belongs_to(monkeypatch):
    # the version heads every runlog and every comlog, so it has to be
    # readable by a suite whose environment has not been sourced -- which is
    # how the code is imported by the test suite and by anything embedding it
    monkeypatch.delenv("TOOLS", raising=False)
    monkeypatch.delenv("QUNEXREPO", raising=False)

    with open(log_module.VERSION_FILE, "r") as version_file:
        expected = version_file.read().strip()

    assert gl.get_qunex_version() == expected
    assert expected in gl.print_qunex_header(timestamp="2024-01-01")


def test_a_version_that_cannot_be_read_is_reported_as_unknown(monkeypatch, tmp_path):
    # a header is not a place to end a run: an installation that keeps the
    # python package away from VERSION.md still gets a log file
    monkeypatch.setattr(log_module, "VERSION_FILE", str(tmp_path / "gone" / "VERSION.md"))
    monkeypatch.setenv("TOOLS", str(tmp_path / "gone"))
    monkeypatch.setenv("QUNEXREPO", "nowhere")

    assert gl.get_qunex_version() == "unknown"
    assert gl.print_qunex_header(timestamp="2024-01-01") == (
        "# Generated by QuNex unknown on 2024-01-01"
    )
