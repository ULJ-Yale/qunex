...
from qx_utilities.general.snapshots import record_snapshot, compare_snapshots, rollback_snapshot
...
def test_record_snapshot_basic():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        lines_with_metadata = [line for line in content.split('\n') if '[' in line and 'bytes]' in line]
...
def test_record_snapshot_without_hash():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print("Snapshot created successfully without hashes")
...
def test_compare_snapshots_added_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        lines = content.split('\n')
        new_file_line = [line for line in lines if "new_file.txt" in line][0]
        assert "+ " in new_file_line, "new_file.txt should be marked as added"

        another_new_line = [line for line in lines if "another_new.txt" in line][0]
...
def test_compare_snapshots_deleted_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        lines = content.split('\n')
        file1_line = [line for line in lines if "file1.txt" in line][0]
