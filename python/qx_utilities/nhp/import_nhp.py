...
def import_nhp(sessionsfolder=None, inbox=None, sessions=None, action="link", overwrite="no", archive="move"):
    """
    ...
    if archive not in ["leave", "move", "copy", "delete"]:
        raise ge.CommandError("import_nhp", "Invalid dataset archive option", "%s is not a valid option for dataset archive option!" % (archive), "Please specify one of: move, copy, delete!")
    ...
    if os.path.exists(inbox):
        if os.path.isfile(inbox):
            ...
            raise ge.CommandFailed("import_nhp", "Invalid inbox", "%s is neither a file or a folder!" % (inbox), "Please check your path!")
    else:
        raise ge.CommandFailed("import_nhp", "Inbox does not exist", "The specified inbox [%s] does not exist!" % (inbox), "Please check your path!")
        ...
        if file.endswith(".zip"):
            ...
            try:
                ...
                for sf in z.infolist():
                    if sf.filename[-1] != "/":
                        result = map_to_qunex(sf.filename, sessionsfolder, sessions, overwrite)
                        if result:
                            tfile = result[1]
                            fdata = z.read(sf)
                            fout = open(tfile, "wb")
            ...
            try:
                ...
                for member in tar.getmembers():
                    if member.isfile():
                        result = map_to_qunex(member.name, sessionsfolder, sessions, overwrite)
                        if result:
                            tfile = result[1]
    ...
    if errors:
        ...
        if os.path.isfile(inbox) or not os.path.samefile(inbox, os.path.join(sessionsfolder, "inbox", "NHP")):
            try:
                if archive == "move":
            ...
            for file in files:
                try:
                    if archive == "move":
                        print("--> moving dataset to archive")
                        shutil.move(file, os.path.join(sessionsfolder, "archive", "NHP"))
                    elif archive == "copy":
                        print("--> copying dataset to archive")
                        shutil.copy2(file, os.path.join(sessionsfolder, "archive", "NHP"))
                    elif archive == "delete":
                        print("--> deleting dataset")
    ...
    if len(report) > 0:
        ...
        for s in report:
            ...
            print("# Generated by QuNex %s on %s" % (gc.get_qunex_version(), datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), file=sout_nhp)
            print("#", file=sout_nhp)
            print('id:', s, file=sout_nhp)
