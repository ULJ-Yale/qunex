...
def hcp_transmit_bias_individual_align(sinfo, options, overwrite=False, thread=0):
    """
    ``hcp_transmit_bias_individual_alin [... processing options]``
    ...
    r += "\n%s HCP Transmit Bias Pipeline Phase 1, Individual Align [%s] ..." % (
        pc.action("Running", options["run"]),
        options["hcp_processing_mode"],
    )
    ...
    try:
        pc.doOptionsCheck(options, sinfo, "hcp_transmit_bias_individual_align")
        doHCPOptionsCheck(options, "hcp_transmit_bias_individual_align")
        hcp = getHCPPaths(sinfo, options)
...
def hcp_transmit_bias_individual_adjustment(sinfo, options, overwrite=False, thread=0):
    ...
    r += "\n%s HCP Transmit Bias Phase 3, Individual Adjustment [%s] ..." % (
        pc.action("Running", options["run"]),
        options["hcp_processing_mode"],
    )
    ...
    try:
        pc.doOptionsCheck(options, sinfo, "hcp_transmit_bias_individual_adjustment")
        doHCPOptionsCheck(options, "hcp_transmit_bias_individual_adjustment")
        hcp = getHCPPaths(sinfo, options)
        ...
        if run:
            ...
            # -- just checking
                ...
                if passed is None:
                    r += "\n---> HCP Transmit Bias Phase 3,Individual Adjustment can be run"
    ...
    r += (
        "\n\nHCP Transmit Bias Phase 3,Individual Adjustment Preprocessing %s on %s\n------------------------------------------------------------"
        % (
            pc.action("completed", options["run"]),
            datetime.now().strftime("%A, %d. %B %Y %H:%M:%S"),
        )
