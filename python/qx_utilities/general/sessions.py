...
def join_session(studyfolder: str, source: str, target: str, overwrite: str = 'no', raw_data: str = 'copy') -> bool:
    ...
    if target_exists:
        ...
        if target_has_content:
            ...
            has_derivatives = (os.path.exists(os.path.join(target_path, 'images')) or
                             os.path.exists(os.path.join(target_path, 'hcp')))
    ...
    if overwrite == 'merge' and target_has_content and not target_is_joined:
        ...
        if os.path.exists(target_nii):
            for item in sorted(os.listdir(target_nii), reverse=True):  # Reverse to avoid conflicts
    ...
    for idx, session_data in enumerate(source_session_data):
        ...
        sequences = session_data.get('hcp_sequences', session_data.get('txt_sequences', []))
        
        # Calculate offset for this session
        if overwrite == 'merge' and target_is_joined:
            # Target is joined, find next available prefix
            # Get the next prefix number (not the offset)
            next_prefix = _find_next_prefix_offset(target_sequences, base_increment) // base_increment
        ...
        if raw_data != 'leave':
            ...
            session_num = idx + (2 if (overwrite == 'merge' and target_has_content and not target_is_joined) else 1)
...
def _merge_metadata(combined: Dict, new_metadata: Dict) -> None:
    ...
    for key, value in new_metadata.get('additional', []):
        if key in existing_keys:
            if value not in existing_keys[key]:
                print(f"WARNING: Metadata conflict for '{key}': "
...
def _write_session_files(
    ...
    if hcp_sequence_lines or existing_target_sequences or hcp_metadata.get('additional'):
