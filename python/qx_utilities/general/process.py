...
arglist = [
    ['# ---- Basic settings'],
    ['batchfile',          '',
        str,   "The file with sessions information."],
    ['sessions',           '',
        str,    "A list of sessions to process."],
    ['sessionsfolder',     '',
        os.path.abspath, 'The path to study sessions folder.'],
    ['logfolder',          '',
        isNone, 'The path to log folder.'],
    ['logtag',             '',                                            str,
        'An optional additional tag to add to the log file after the command name.'],
    ['overwrite',          'no',
        torf,   'Whether to overwrite existing results.'],
    ['parsessions',        '1',                                           int,
        'How many processor sessions to run in parallel.'],
    ['parelements',        '1',
        int,    'How many elements to run in parralel.'],
    ['nprocess',           '0',                                           int,
        'How many sessions to process (0 - all).'],
    ['datainfo',           'False',
        torf,   'Whether to print information.'],
    ['printoptions',       'False',
        torf,   'Whether to print options.'],
    ['filter',             '',
        str,    'Filtering information.'],
    ['script',             '',
        isNone, 'The script to be executed.'],
    ['sessionid',          '',                                            str,
        "a session id for which to run the command"],
    ['sessionids',         '',                                            str,
        "list of | separated session ids for which to run the command"],

    ['# ---- Preprocessing options'],
    ['bet',                '-f 0.5',                                      str,
        "options to be passed to BET in brain extraction"],
    ...
    ['tr',                 '2.5',
        float,  "TR of the bold data"],
    ['omit',               '5',                                           int,
        "how many frames to omit at the start of each bold run"],
    ...
    ['boldname',           'bold',
        str,    "the default name for the bold files"],
    ...
    ['pignore',            '',
        str,    "what to do with frames marked as bad"],
    ['event_file',         '',                                            str,
        "the root name of the fidl event file for task regression"],
    ['event_string',       '',                                           str,
        "string specifying what and how of task to regress out"],
    ['source_folder',      'True',
        torf,   "hould we check for source folder (yes/no)"],
    ['wbmask',             '',                                            str,
        "mask specifying what ROI to exclude from WB mask"],
    ['sessionroi',         '',                                            str,
        "a mask used to specify session specific WB"],
    ['nroi',               '',                                            str,
    ...
    ['path_bold',          'bold[N]/*faln_dbnd_xr3d_atl.4dfp.img',
        str,    "the mask to use for searching for bold images"],
    ['path_mov',           'movement/*_b[N]_faln_dbnd_xr3d.dat',
        str,    "the mask to use for searching for movement files"],
    ['path_t1',            'atlas/*_mpr_n*_111_t88.4dfp.img',
        str,    "the mask to use for searching for T1 file"],
    ...
    ['image_atlas',        'cifti',
        str,    "what is the target atlas (711, cifti)"],
    ...
    ['mov_radius',          '50.0',
        float,  "the assumed radius of the brain"],
    ['mov_scrub',           'yes',                                         str,
    ...
    ['mov_before',          '0',                                           int,
        "how many frames preceeding bad frames to also exclude"],
    ['mov_after',           '0',                                           int,
        "how many frames following bad frames to also exclude"],
    ...
    ['mov_mreport',         'movement_report.txt',
        str,    "the name of the movement report file"],
    ['mov_preport',         'movement_report_post.txt',
        str,    "the name of the post scrub movement report file"],
    ['mov_sreport',         'movement_scrubbing_report.txt',
        str,    "the name of the scrubbing report file"],
    ['mov_pdf',             'movement_plots',                              str,
        "the name of the folder that holds movement stats plots"],
    ['mov_pref',            "",
        str,    "the prefix for the movement report files"],

    ['# ---- CIFTI related options'],
    ['surface_smooth',      '2.0',
        float,  "sigma for cifti surface smoothing"],
    ['volume_smooth',       '2.0',
        float,  "sigma for cifti volume smoothing"],
    ['voxel_smooth',        '1',
        float,  "extent of volume smoothing in voxels"],
    ...
    ['hipass_filter',       '0.008',
        float,  "highpass filter to use"],
    ['lopass_filter',       '0.09',
        float,  "lopass filter to use"],
    ['hipass_do',           'nuisance',
        str,    "What to high-pass filter besides BOLD signal"],
    ['lopass_do',           'nuisance,movement,events,task',
        str,    "What to low-pass filter besides BOLD signal"],
    ['omp_threads',         '0',                                           int,
        "number of cores to be used in wb_command (0 - don't change system settings)"],
    ['framework_path',      '',                                            str,
        "the path to framework libraries on mac system"],
    ['wb_command_path',     '',
        str,    "the path to wb_command"],
    ...
    ['hcp_freesurfer_home',    '',
        str,    "path to FreeSurfer base folder."],
    ['hcp_freesurfer_module',  '',                                         str,
        "Whether to load FreeSurfer as a module on the cluster: YES or NONE."],
    ['hcp_suffix',             '',                                         str,
        "session id suffix if running HCP preprocessing variants."],
    ['hcp_t2',                 't2',                                       str,
        "whether T2 image is present - anything or NONE."],
    ['hcp_printcom',           '',                                         str,
    ...
    ['hcp_lowresmesh',         '32',
        str,    "Usually 32 vertices."],
    ['hcp_lowresmeshes',       '32',
        str,    "Usually 32 vertices."],
    ['hcp_hiresmesh',          '164',
        int,    "Usually 164 vertices."],
    ['hcp_bold_res',           '2',
        str,    "Target image resolution 2mm recommended."],
    ['hcp_grayordinatesres',   '2',
        int,    "Usually 2mm."],
    ['hcp_regname',            'MSMSulc',                                  str,
        "What registration is used FS or MSMSulc. FS if none is provided."],
    ['hcp_fs_longitudinal',    '',                                         str,
    ...
    ['hcp_brainsize',          '150',
        int,    "Human brain size in mm."],
    ['hcp_t1samplespacing',    '',                                         str,
        "0.0000074 ... DICOM field (0019,1018) in s or '' if not used."],
    ['hcp_t2samplespacing',    '',                                         str,
    ...
    ['hcp_bfsigma',            '',
        str,    "Bias Field Smoothing Sigma (optional)."],
    ['hcp_avgrdcmethod',       'NONE',                                     str,
    ...
    ['hcp_prefs_template_res', '0.7',                                      str,
    ...
    ['hcp_bold_smoothFWHM',    '2',
        int,    "The size of the smoothing kernel (in mm)."],
    ...
    ['hcp_prefs_templatemask', '',
        isNone, "Path to the template mask."],
    ['hcp_prefs_template2mmmask', '',
        isNone, "Path to the 2mm template mask."],
    ['hcp_prefs_fnirtconfig', '',
        isNone, "Path to the FNIRT config."],
    ...
    ['hcp_control_points',     '',
        str,    "Whether to run with manual control points."],
    ['hcp_wm_edits',           '',                                         str,
        "Whether to run with manually edited WM mask file."],
    ['hcp_autotopofix_off',    '',                                         str,
        "YES to turn off the automatic topologic fix step in FS and compute WM surface deterministically from manual WM mask (empty)."],
    ['hcp_fs_brainmask',       '',                                         str,
        "Specify 'original' to keep the masked original brainimage; 'manual' to use the manually edited brainmask file; default 'fs'uses the brainmask generated by mri_watershed [fs]."],
    ...
    ['hcp_inflatescale',       '1',
        str,    "Inflate extra scale parameter [1]."],
    ...
    ['hcp_dwi_echospacing',    '',
        str,    "Echo spacing in msec."],
    ['hcp_dwi_phasepos',       'PA',                                       str,
        "The direction of unwarping for positive phase."],
    ...
    ['hcp_dwi_name',           '',
        isNone, "Name to give DWI output directories."],
    ['hcp_dwi_nogpu',          None,                                       flag,
    ...
    ['hcp_postfix_reusehighpass', 'TRUE',
        torf,   "Whether to reuse highpass."],
    ...
    ['hcp_msmall_outregname',  'MSMAll_InitialReg',
        str,    "Output registration name."],
    ['hcp_msmall_procstring',  '',
        isNone, "Identification for FIX cleaned dtseries to use."],
    ...
    ['hcp_resample_concatregname', 'MSMAll',
        str,    "Output name of the dedrifted registration."],
    ['hcp_resample_regname',   '',
        isNone, "Registration sphere name."],
    ...
    ['hcp_resample_myelinmaps', 'MyelinMap,SmoothedMyelinMap',
        str,    "Comma separated paths to myelin maps."],
    ...
    ['hcp_task_lvl1tasks', '',
        isNone, "Comma separated list of task fMRI scan names."],
    ['hcp_task_lvl1fsfs', '',
        isNone, "Comma separated list of of design names."],
    ...
    ['hcp_bold_final_smoothFWHM', '',
        isNone, "Value (in mm FWHM) of total desired smoothing."],
    ...
    ['hcp_asl_territories_atlas', '',
        isNone,  "Atlas of vascular territories from Mutsaerts."],
    ['hcp_asl_territories_labels', '',
        isNone,  "Labels corresponding to territories_atlas."],
    ...
    ['hcp_tica_outfmriname',  'rfMRI_REST',
        str,     "Name to use for tICA pipeline outputs."],
    ...
    ['hcp_outgroupname', '',
        isNone,  "Name to use for the group output folder."],
    ...
    ['hcp_tica_pca_internal_dim', '',
        isNone,  "Override internal MIGP dimensionality."],
    ...
    ['hcp_grayordinates_dir',        '',
        isNone,  "Path to the location of the standard grayorinates space."],
    ['hcp_free_surfer_labels',       '',
        isNone,  "Path to the location of the FreeSurfer look up table file."],
    ['hcp_pregradient_smoothing',    '1',
        int,     "Sigma of the pregradient smoothing."],
    ['hcp_mad_regname',              'MSMAll',
        str,     "Name of the registration."],
    ['hcp_mad_videen_maps',          'corrThickness,thickness,MyelinMap_BC,SmoothedMyelinMap_BC',
        str,     "Maps you want to use for the videen palette."],
    ['hcp_mad_greyscale_maps',       'sulc,curvature',
        str,     "Maps you want to use for the greyscale palette."],
    ['hcp_mad_distortion_maps',      'SphericalDistortion,ArealDistortion,EdgeDistortion',
        str,     "Distortion maps."],
    ['hcp_mad_gradient_maps',        'MyelinMap_BC,SmoothedMyelinMap_BC,corrThickness',
        str,     "Maps you want to compute the gradietn on."],
    ['hcp_mad_std_maps',             'sulc@curvature,corrThickness,thickness,MyelinMap_BC',
        str,     "Maps you want to compute the standard deviation on."],
    ...
    ['# --- Processing options'],
    ['run',                    'run',                                      str,
        "Run type: run - do the task, test - perform checks."],
    ['log',                    'keep',                                     str,
    ...
    ['fix_aggressive_cleanup',     '',       flag,
        "A flag for performing aggressive celanup."],
    ...
    ['hcp_long_fs_template_id', 'base',
        str,     "Name of the base template."],
    ['hcp_long_fs_extra_reconall_base',  '',                                isNone,
    ...
    ['hcp_long_fs_extra_reconall',  '',                                     isNone,
...
flaglist = [
    ['test',                        'run',
        'test', 'Run a test only.'],
    ['hcp_dwi_nogpu',               'hcp_dwi_nogpu',                True,
        'If specified, use the non-GPU-enabled version of eddy. Defaults to using the GPU-enabled version of eddy.'],
    ['hcp_dwi_selectbestb0',        'hcp_dwi_selectbestb0',         True,
        "If set selects the best b0 for each phase encoding direction to pass on to topup rather than the default behaviour of using equally spaced b0's throughout the scan. The best b0  is identified as the least distorted (i.e., most similar to the average b0 after registration). The flag is not set by default."],
    ['hcp_asl_use_t1',              'hcp_asl_use_t1',               True,
        'If specified, the T1 estimates from the satrecov model fit will be used in perfusion estimation in oxford_asl.'],
    ['hcp_asl_nobandingcorr',       'hcp_asl_nobandingcorr',        True,
        'If this option is provided, MT and ST banding corrections will not be applied.'],
    ['hcp_task_vba',                'hcp_task_vba',
        True,   "VBA YES/NO."],
    ['hcp_tica_config_out',         'hcp_tica_config_out',          False,
        "Generate config file for rerunning with similar settings, or for reusing these results for future cleaning."],
    ['no_despike',                  'no_despike',
        True,   "Provide this to disable despiking."],
    ['fix_no_motion_cleanup',       'fix_no_motion_cleanup',        True,
        "Provide this to disable cleanup of motion confounds."],
    ['fix_aggressive_cleanup',      'fix_aggressive_cleanup',
        False,  "Provide this to enable agressive cleanup."],
]
...
# processing commands
calist = [
    ['mhd',     'map_hcp_data',             process_hcp.map_hcp_data,
        "Map HCP preprocessed data to sessions' image folder."],
    [],
    ['gbd',     'get_bold_data',            workflow.get_bold_data,
        "Copy functional data from 4dfp (NIL) processing pipeline."],
    ['bbm',     'create_bold_brain_masks',  workflow.create_bold_brain_masks,
        "Create brain masks for BOLD runs."],
    [],
    ['seg',     'run_basic_segmentation',   fs.runBasicStructuralSegmentation,
        "Run basic structural image segmentation."],
    ['gfs',     'get_fs_data',              fs.checkForFreeSurferData,
        "Copy existing FreeSurfer data to sessions' image folder."],
    ['fss',     'run_subcortical_fs',       fs.runFreeSurferSubcorticalSegmentation,
        "Run subcortical freesurfer segmentation."],
    ['fsf',     'run_full_fs',              fs.runFreeSurferFullSegmentation,
        "Run full freesurfer segmentation"],
    [],
    ['cbs',     'compute_bold_stats',       workflow.compute_bold_stats,
        "Compute BOLD movement and signal statistics."],
    ['csr',     'create_stats_report',      workflow.create_stats_report,
        "Create BOLD movement statistic reports and plots."],
    ['ens',     'extract_nuisance_signal',  workflow.extract_nuisance_signal,
        "Extract nuisance signal from BOLD images."],
    [],
    ['bpp',     'preprocess_bold',          workflow.preprocess_bold,
        "Preprocess BOLD images (using old Matlab code)."],
    ['cpp',     'preprocess_conc',          workflow.preprocess_conc,
        "Preprocess conc bundle of BOLD images (using old Matlab code)."],
    [],
    ['hcp1',    'hcp_pre_freesurfer',       process_hcp.hcp_pre_freesurfer,
        "Run HCP PreFS pipeline."],
    ['hcp2',    'hcp_freesurfer',           process_hcp.hcp_freesurfer,
        "Run HCP FS pipeline."],
    ['hcp3',    'hcp_post_freesurfer',      process_hcp.hcp_post_freesurfer,
        "Run HCP PostFS pipeline."],
    ['hcp4',    'hcp_fmri_volume',          process_hcp.hcp_fmri_volume,
        "Run HCP fMRI Volume pipeline."],
    ['hcp5',    'hcp_fmri_surface',         process_hcp.hcp_fmri_surface,
        "Run HCP fMRI Surface pipeline."],
    ['hcp6',    'hcp_icafix',               process_hcp.hcp_icafix,
        "Run HCP ICAFix pipeline."],
    ['hcp7',    'hcp_post_fix',             process_hcp.hcp_post_fix,
        "Run HCP PostFix pipeline."],
    ['hcp8',    'hcp_reapply_fix',          process_hcp.hcp_reapply_fix,
        "Run HCP ReApplyFix pipeline."],
    ['hcp9',    'hcp_msmall',               process_hcp.hcp_msmall,
        "Run HCP MSMAll pipeline."],
    ['hcp10',   'hcp_dedrift_and_resample', process_hcp.hcp_dedrift_and_resample,
        "Run HCP DeDriftAndResample pipeline."],
    ['hcp11',   'hcp_task_fmri_analysis',   process_hcp.hcp_task_fmri_analysis,
    ...
    ['premice', 'preprocess_mice',          qx_mice.process_mice.preprocess_mice,
...
lalist = [
    ['hcp_lfs', 'hcp_longitudinal_freesurfer',  process_hcp.hcp_longitudinal_freesurfer,
        "Runs longitudinal FreeSurfer across sessions."]
]

# multi-session commands
malist = [
    ['hpc_tica',    'hcp_temporal_ica',         process_hcp.hcp_temporal_ica,
        "Run HCP temporal ICA pipeline."],
    ['hcp_mad',     'hcp_make_average_dataset', process_hcp.hcp_make_average_dataset,
        "Run HCP make average dataset pipeline."],
]

salist = [
    ['cbl',     'create_bold_list',
        simple.create_bold_list,                        "Create BOLD list"],
    ['ccl',     'create_conc_list',
        simple.create_conc_list,                        "Create conc list"],
    ['lsi',     'list_session_info',
        simple.list_session_info,                       "List session info"]
]
...
def run(command, args):
    ...
    sessions, gpref = gc.get_sessions_list(
        options['sessions'], filter=options['filter'], sessionids=options['sessionids'], verbose=False)
    ...
    if command in lactions:
        ...
        for session in sessions:
            if 'subject' not in session:
                raise ge.CommandFailed(command, "Missing subject information", "%s batch file does not provide subject information for session id %s." % (
                    options['subjects'], session['id']), "Please check the batch file!", "Aborting processing!")
    ...
    for line in arglist:
        if len(line) == 4:
            try:
                ...
                raise ge.CommandError(command, "Invalid parameter value!", "Parameter `%s` is specified but is set to an invalid value:" % (
                    line[0]), '--> %s=%s' % (line[0], str(options[line[0]])), "Please check acceptable inputs for %s!" % (line[0]))
    ...
    if options['scheduler'] == 'local':
        ...
        if parsessions == 1 or options['run'] == 'test':
                ...
                for session in sessions:
                    if len(session['id']) > 1:
                        ...
                        consoleLog += "\nStarting %s of sessions %s at %s" % (
                            action, session['id'], datetime.now().strftime("%A, %d. %B %Y %H:%M:%S"))
                        print("\nStarting %s of sessions %s at %s" % (
                            action, session['id'], datetime.now().strftime("%A, %d. %B %Y %H:%M:%S")))
                        r, status = procResponse(pending_actions(
                            session, soptions, overwrite, c + 1))
                        writelog(r)
                        consoleLog += r
                        print(r)
                        stati.append(status)
            ...
            # multi-session commands
                ...
                # log
                consoleLog += "\nStarting %s of sessions %s at %s" % (
                    action, sessionids, datetime.now().strftime("%A, %d. %B %Y %H:%M:%S"))
                print("\nStarting %s of sessions %s at %s" % (
                    action, sessionids, datetime.now().strftime("%A, %d. %B %Y %H:%M:%S")))

                # process
                r, status = procResponse(pending_actions(
                    sessions, sessionids, soptions, overwrite, c + 1))
            ...
            # longitudinalo commands
                ...
                consoleLog += "\nStarting %s of subjects %s at %s" % (
                    action, subjectids, datetime.now().strftime("%A, %d. %B %Y %H:%M:%S"))
                print("\nStarting %s of subjects %s at %s" % (
                    action, subjectids, datetime.now().strftime("%A, %d. %B %Y %H:%M:%S")))

                # process
                r, status = procResponse(pending_actions(
                    sessions, subjectids, soptions, overwrite, c + 1))
            ...
            # simple processing commands
                ...
                r, status = procResponse(
                    pending_actions(sessions, soptions, overwrite))
                writelog(r)
                ...
                for session in sessions:
                    if len(session['id']) > 1:
                        ...
                        consoleLog += "\nAdding processing of session %s to the pool at %s" % (
                            session['id'], datetime.now().strftime("%A, %d. %B %Y %H:%M:%S"))
                        print("\nAdding processing of session %s to the pool at %s" % (
                            session['id'], datetime.now().strftime("%A, %d. %B %Y %H:%M:%S")))
        ...
        print("\n\n===> Final report for command",
              options['command_ran'], file=f)
        failedTotal = 0
    ...
    #                                                  general scheduler code
        ...
        # schedule
        gs.runThroughScheduler(command, sessions=sessions, args=args, parsessions=parsessions,
                               logfolder=os.path.join(logfolder, 'batchlogs'), logname=logname)
