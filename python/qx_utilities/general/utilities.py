...
def manage_study(studyfolder=None, action="create", folders=None, verbose=False):
    ...
    if create:
        ...
        # --> parameter template
        paramFile = os.path.join(studyfolder, 'sessions', 'specs', 'parameters_example.txt')
        try:
            ...
            if e.errno == errno.EEXIST:
                ...
                errstr = os.strerror(e.errno)
                raise ge.CommandFailed("manage_study", "I/O error: %s" % (errstr), "Parameters template file could not be created [%s]!" % (paramFile), "Please check paths and permissions!")
        ...
        try:
            f = os.open(markFile, os.O_CREAT|os.O_EXCL|os.O_WRONLY)
            os.write(f, bytes("%s study folder created on %s by %s." % (os.path.basename(studyfolder), datetime.now().strftime("%Y-%m-%d %H:%M:%S"), username), encoding="utf8"))
            os.close(f)
            if verbose:
...
def create_study(studyfolder=None, folders=None):
    ...
    parameters_example.txt and pipeline example mapping files in
...
def create_batch(sessionsfolder=".", sourcefiles=None, targetfile=None, sessions=None, filter=None, overwrite="no", paramfile=None):
    ...
    Note that if a batch file already exists then parameter file will not be 
    ...
    try:
        ...
        if overwrite == 'yes':
            ...
            # header
            print("# Generated by QuNex %s on %s" % (gc.get_qunex_version(), datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), file=jfile)
            print("#", file=jfile)
        ...
        if overwrite == 'yes' or not preexist:
            if paramfile is None:
                ...
                if not os.path.exists(paramfile):
                    ...
                    for line in gp.arglist:
                        if len(line) == 4:
                            print("# _%-24s : %-15s ... %s" % (line[0], line[1], line[3]), file=pfile)
                        elif len(line) > 0:
                            print("#\n# " + line[0] + '\n#', file=pfile)
                    pfile.close()
...
def create_list(sessionsfolder=".", sessions=None, filter=None, listfile=None, bolds=None, conc=None, fidl=None, glm=None, roi=None, boldname="bold", bold_tail=".nii.gz", img_suffix="", bold_variant="", overwrite='no', check='yes'):
    ...
    for session in sessions:
        ...
        if boldtags:
            try:
                bolds = [(bsearch.match(v['name']).group(1), v['name'], v['task']) for (k, v) in session.items() if k.isdigit() and bsearch.match(v['name'])]
    ...
    if overwrite == 'yes':
        ...
        print("# Generated by QuNex %s on %s" % (gc.get_qunex_version(), datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), file=lfile)
        print("#", file=lfile)
...
def create_conc(sessionsfolder=".", sessions=None, filter=None, concfolder=None, concname="", bolds=None, boldname="bold", bold_tail=".nii.gz", img_suffix="", bold_variant="", overwrite='no', check='yes'):
    ...
    for session in sessions:
        ...
        if boldtags:
            try:
                bolds = [(int(bsearch.match(v['name']).group(1)), v['name'], v['task']) for (k, v) in session.items() if k.isdigit() and bsearch.match(v['name'])]
                if "all" not in boldtags:
                    bolds = [n for n, b, t in bolds if t in boldtags]
                else:
                    bolds = [n for n, b, t in bolds]
...
def run_list(listfile=None, runlists=None, logfolder=None, verbose="no", eargs=None):
    ...
    for runListName in runLists:
        ...
        for runCommand in commandsToRun:
            ...
            if error:
                ...
                print("\n---> Running lists not completed successfully: failed running command '%s' in list '%s'" % (commandName, runListName), file=log)
                log.close()
    ...
    print(summary, file=log)
    print("\n===> Successful completion of task: run_lists %s" % (", ".join(runLists)), file=log)
...
def gather_behavior(sessionsfolder=".", sessions=None, filter=None, sourcefiles="behavior.txt", targetfile=None, overwrite="no", check="yes", report="yes"):
    ...
    if report:
        print("# Data compiled using gather_behavior on %s" % (datetime.today()), file=fout)
...
def pull_sequence_names(sessionsfolder=".", sessions=None, filter=None, sourcefiles="session.txt", targetfile=None, overwrite="no", check="yes", report="yes"):
    ...
    if report:
        print("# Data compiled using pull_sequence_names on %s" % (datetime.today()), file=fout)
