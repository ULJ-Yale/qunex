...
def manage_study(studyfolder=None, action="create", folders=None, verbose=False):
    ...
    if create:
        ...
        try:
            ...
            os.write(f, bytes("# Generated by QuNex %s on %s\n" % (gc.get_qunex_version(
            ), datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), encoding="utf8"))
            os.write(f, bytes("#\n", encoding="utf8"))
    ...
    try:
        ...
        if overwrite == 'yes':
            ...
            print("# Generated by QuNex %s on %s" % (gc.get_qunex_version(),
                  datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), file=jfile)
            print("#", file=jfile)
...
def create_list(sessionsfolder=".", sessions=None, sessionids=None, filter=None, listfile=None, bolds=None, conc=None, fidl=None, glm=None, roi=None, boldname="bold", bold_tail=".nii.gz", img_suffix="", bold_variant="", overwrite='no', check='yes'):
    ...
    if overwrite == 'yes':
        ...
        print("# Generated by QuNex %s on %s" % (gc.get_qunex_version(),
              datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), file=lfile)
        print("#", file=lfile)
    ...
    for pipeline in pipelines:
        ...
        for sfolder in sfolders:
            ...
            try:
                ...
                # qunex header
                print("# Generated by QuNex %s on %s" % (gc.get_qunex_version(
                ), datetime.now().strftime("%Y-%m-%d_%H.%M.%S.%f")), file=fout)
                print("#", file=fout)
