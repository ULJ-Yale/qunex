...
json_mapping = {
    "phenc": ["PhaseEncodingDirection", lambda x: pe_dir_map.get(x, "NA")],
...
def map_to_qunex_bids(
    ...
    bids_structure = os.path.join(niu_template_folder, "import_bids.txt")

    if not os.path.exists(bids_structure):
        raise ge.CommandFailed(
            "map_to_qunex_bids",
            "No BIDS structure file present!",
            "There is no BIDS structure file %s" % (bids_structure),
    ...
    for part in re.split(r"_|/|\.", bids_path):
        if part.startswith("sub-"):
            ...
            for target_modality in bids["modalities"]:
                if part in bids[target_modality]["label"]:
    ...
    if session in sessions_list["skip"]:
        return False, False

    # ---> processing a new session
    elif session not in sessions_list["list"]:
        sessions_list["list"].append(session)
...
def import_bids(
    ...
    if os.path.exists(inbox):
        if os.path.isfile(inbox):
            ...
            if folder_type in studyat:
                metadata_path = os.path.join(
                    "/", *inbox.split(os.path.sep)[: studyat[folder_type]]
                )
            else:
                metadata_path = inbox
            ...
            mcandidates = []
            for m in metadata:
                mcandidates += glob.glob(os.path.join(metadata_path, m))

            for mcandidate in mcandidates:
                if os.path.isfile(mcandidate):
                    source_files.append(mcandidate)
    ...
    if bidsname is None:
        ...
        elif folder_type in ["inbox", "bids_study"]:
            bidsname = os.path.basename(inbox)
        elif folder_type in ["subject", "session"]:
            bidsname = inbox.split(os.path.sep)[studyat[folder_type] - 1]
    ...
    if not all_ok:
        ...
        # -> we're archiving fully processed inbox folder
            ...
            if os.path.samefile(inbox, bidsinbox):
                archive_list = glob.glob(os.path.join(inbox, "*"))
        ...
        if archive in ["move", "copy"]:
            if folder_type == "file":
                archive_folder = bidsarchive
            elif folder_type in ["inbox", "bids_study"]:
                if os.path.samefile(inbox, bidsinbox):
                    archive_folder = bidsarchive
                else:
                    archive_folder = os.path.join(bidsarchive, os.path.basename(inbox))
            else:
                if os.path.samefile(inbox, bidsinbox):
                    archive_folder = os.path.join(
                        bidsarchive, *inbox.split(os.path.sep)[studyat[folder_type] :]
                    )
                else:
                    archive_folder = os.path.join(
                        bidsarchive,
                        *inbox.split(os.path.sep)[studyat[folder_type] - 1 :],
                    )
        ...
        for archive_item in archive_list:
            # -> delete items
            if archive == "delete":
                if os.path.isfile(archive_item):
                    io = fl.remove(archive_item)
                else:
            ...
            # -> move or copy items
            if archive in ["move", "copy"]:
                target_item = archive_item.replace(inbox, "")
                target_item = re.sub(r"^%s+" % (os.path.sep), "", target_item)
                archive_target = os.path.join(archive_folder, target_item)
                archive_target_folder = os.path.dirname(archive_target)
                ...
                fl.lock(archive_target)
                try:
                    if archive == "move":
                        shutil.move(archive_item, archive_target_folder)
                    else:
                        if os.path.isfile(archive_item):
                            shutil.copy2(archive_item, archive_target_folder)
                        else:
    ...
    if sessions_list["bids"] == "locked":
        bids_info_status = fl.wait_status(
...
def process_bids(bfolder):
    ...
    for sfile in source_files:
        ...
        if modality:
            modality = modality[0]
            if modality not in bids_data[session]:
                bids_data[session][modality] = []
            ...
            bids_data[session][modality].append(info)
        else:
            bids_data[session]["files"] = {
                "filepath": sfile,
                "filename": filename,
                "json_file": sidecar,
                "json_info": sideinfo,
    ...
    for session in bids_data:
        bids_data[session]["images"] = {"list": [], "info": {}}
        imgn = 0
        for modality in ["anat", "fmap", "func", "dwi", "asl"]:
            if modality in bids_data[session]:
                for element in bids_data[session][modality]:
    ...
    for session in bids_data:
        basenames = []
        for element in bids_data[session].get("fmap", []):
            ...
            if ".nii" in element["filename"]:
                ...
                if (
                    ...
                    for target_file in target_files:
                        ...
                        if tag not in bids_data[session]["images"]["info"][
                            target_file
                        ].get("seq_info", []):
                            bids_data[session]["images"]["info"][target_file][
                                "seq_info"
                            ] = bids_data[session]["images"]["info"][target_file].get(
                                "seq_info", []
                            ) + [tag]
...
def _sort_bids_images(bids_data, bids):
    """
        ...
        for modality in bids["modalities"]:
            if modality in bids_data[session]:
                for key in bids[modality]["sort"]:
                    if key == "label":
                        bids_data[session][modality].sort(
                            key=lambda x: _label_order(bids[modality]["label"], x[key])
                        )
                    else:
                        bids_data[session][modality].sort(key=lambda x: x[key] or "")
...
def map_bids2nii(
    ...
    bids_data = bids_data[session]

    if not bids_data["images"]["list"]:
        raise ge.CommandFailed(
            "map_bids2nii",
    ...
    for image in bids_data["images"]["list"]:
        imgn = bids_data["images"]["info"][image]["imgn"]
        tfile = os.path.join(nfolder, "%d.nii.gz" % (imgn))

        status = gc.move_link_or_copy(
            bids_data["images"]["info"][image]["filepath"], tfile, action="link"
        )
        if bids_data["images"]["info"][image]["json_file"]:
            status = status & gc.move_link_or_copy(
                bids_data["images"]["info"][image]["json_file"],
        ...
        if status:
            ...
            seq_info = ":".join(bids_data["images"]["info"][image].get("seq_info", []))
            if seq_info:
                seq_info = ":" + seq_info

            # ---> check if there is json info present
            json_info = []
            json_data = bids_data["images"]["info"][image].get("json_info", None)
            ...
            if fileinfo == "short":
                file_info = bids_data["images"]["info"][image]["tag"]
            elif fileinfo == "full":
                file_info = (
                    bids_data["images"]["info"][image]["filename"]
                    .replace(".nii.gz", "")
            ...
            print(
                "%s => %s" % (bids_data["images"]["info"][image]["filepath"], tfile),
                file=bout,
            )
        else:
            all_ok = False
            ...
            print(
                "FAILED: %s => %s"
                % (bids_data["images"]["info"][image]["filepath"], tfile),
                file=bout,
            )

        status = True
        if bids_data["images"]["info"][image]["label"] == "dwi":
            sbvec = bids_data["images"]["info"][image]["filepath"].replace(
                ".nii.gz", ".bvec"
            )
            tbvec = tfile.replace(".nii.gz", ".bvec")
            if gc.move_link_or_copy(sbvec, tbvec, action="link"):
                print("%s => %s" % (sbvec, tbvec), file=bout)
            else:
                status = False

            sbval = bids_data["images"]["info"][image]["filepath"].replace(
                ".nii.gz", ".bval"
            )
            tbval = tfile.replace(".nii.gz", ".bval")
            if gc.move_link_or_copy(sbval, tbval, action="link", status=status):
...
def map_nii2bids(
    ...
    if action not in ["hardlink", "copy"]:
        raise ge.CommandError(
            "map_nii2bids",
            "Invalid option for action specified",
    ...
    for session_info in sinfo:
        ...
        print(
            "nii to BIDS mapping report, executed on %s"
            % (datetime.now().strftime("%Y-%m-%dT%H:%M:%S")),
            file=bout,
        )
        ...
        [
            print(f"... mapping subject {subject}, session {session}", file=output)
        ...
        if subject in remaps.keys():
            if session in remaps[subject].keys():
                ...
                [
                    print(
                        f"... remapping session {session} to {sessionid}", file=output
                    )
                    for output in [sys.stdout, bout]
                ]
            else:
            ...
            if not sessionid.isalnum():
                [
                    print(
                        f"... WARNING: session name contains non-alphanumeric characters, replacing them with {replacement}",
                        file=output,
                    )
                    for output in [sys.stdout, bout]
                ]
            ...
            if bidsfiles > 0:
                if overwrite in ["yes", True, "True"]:
                    ...
                    raise ge.CommandFailed(
                        "map_nii2bids",
                        "Existing files present!",
                        "There are existing files in the bids folder [%s]"
                        % (bidsfolder),
                        "Please check or set parameter 'overwrite' to yes!",
                    )
        else:
        ...
        if os.path.exists(os.path.join(sourcefolder, "session_hcp.txt")):
            ...
            raise ge.CommandFailed(
                "map_nii2bids",
                "session_hcp.txt is missing!",
                "Please prepare hcp_mapping.txt file and run create_session_info to prepare 'session_hcp.txt' files",
            )
        ...
        for image_id in images_ids:
            ...
            if image_type[:7] == "boldref":
                ...
                [
                    print(
                        f"... ERROR: image type is empty for image {image_id}, skipping",
                        file=output,
                    )
                    for output in [sys.stdout, bout]
                ]
                ...
                [
                    print(
                        f"... ERROR: image type {image_type} does not exists, skipping",
                        file=output,
                    )
                    for output in [sys.stdout, bout]
                ]
            ...
            files_to_map = [
                filename
                for filename in os.listdir(niifolder)
                if filename.startswith(f"{image_id}.")
            ]
            if len(files_to_map) == 0:
                [
                    print(
                        f"... WARNING: no files found for id: {image_id}, skipping",
                        file=output,
                    )
                    for output in [sys.stdout, bout]
                ]
            ...
            for file_to_map in files_to_map:
                ...
                bids_name, errors_ = _create_bids_name(
                    subject, image_info, image_type, suffix, extension, sessionid, bout
                )
                errors += errors_
                ...
                if bids_name is None:
                    [
                        print(
                            f"... ERROR: could not create BIDS name for {file_to_map}, skipping",
                            file=output,
                        )
                        for output in [sys.stdout, bout]
                    ]
                ...
                if os.path.exists(target_file):
                    [
                        print(
                            f"... WARNING: tried to map {file_to_map} to file {bids_name}, but this target already exists in {targetfolder}, check if mapping is unique",
                            file=output,
                        )
                        for output in [sys.stdout, bout]
                    ]
                ...
                [
                    print(
                        f"... linking {file_to_map} to {os.path.basename(targetfolder)}/{bids_name}",
                        file=output,
                    )
                    for output in [sys.stdout, bout]
                ]
        ...
        if errors > 0:
            raise ge.CommandFailed(
                ...
                f"Errors occured during mapping, please check log file {os.path.join(bidsfolder, 'nii2bids.log')} for details",
            )
...
def _create_bids_name(
    subject, image_info, image_type, suffix, extension, sessionid=None, report=None
):
    ...
    raw_rules = bids_schema["rules"]["files"]["raw"][
        bids_mri_types[image_type]["folder"]
    ]

    entities_order = bids_schema["rules"]["entities"]
    entities_description = bids_schema["objects"]["entities"]
    entities_short_names = {
        key: value["name"] for key, value in entities_description.items()
    }
    entities_format = {
        key: value["format"] for key, value in entities_description.items()
    }
    ...
    allowed_extensions = [
        i[1:] for i in allowed_extensions if i[0] == "."
    ]  # remove dot from extensions
    if extension not in allowed_extensions:
        [
            print(
                f"    ... ERROR: extension {extension} not allowed for {image_type}, skipping",
                file=output,
            )
            for output in [sys.stdout, report]
        ]
        errors += 1
        return (None, errors)
    ...
    if suffix not in allowed_suffixes:
        [
            print(
                f"    ... ERROR: suffix {suffix} not allowed for {image_type}, skipping",
                file=output,
            )
            for output in [sys.stdout, report]
        ]
    ...
    for entity in entities_order:
        if entity in image_info:
            ...
            image_type in ["SE-FM-PA", "SE-FM-AP", "SE-FM-RL", "SE-FM-LR"]
        ...
        if entity not in allowed_entities.keys():
            [
                print(
                    f"    ... WARNING: entity {entity} not allowed for {image_type}",
                    file=output,
                )
                for output in [sys.stdout, report]
            ]
        ...
        if entity_format == "label" and not entity_value.isalnum():
            [
                print(
                    f"    ... ERROR: value {entity_value} for entity {entity} is not alphanumeric for {image_type}, skipping",
                    file=output,
                )
                for output in [sys.stdout, report]
            ]
        ...
        if entity_format == "index" and not entity_value.isdigit():
            [
                print(
                    f"    ... ERROR: value {entity_value} for entity {entity} is not an integer for {image_type}, skipping",
                    file=output,
                )
                for output in [sys.stdout, report]
            ]
    ...
    required_entities = [
        i for i in allowed_entities if allowed_entities[i] == "required"
    ]
    for required_entity in required_entities:
        ...
        if entity_key not in bids_name:
            [
                print(
                    f"    ... ERROR: required entity {required_entity} not found for {image_type}, skipping",
                    file=output,
                )
                for output in [sys.stdout, report]
            ]
...
def map_bids_to_behavior(sfolder=".", behavior=[], overwrite="no"):
    """ """
    ...
    splash = "Running map_bids_to_behavior for %s" % (info)
    print(splash)
