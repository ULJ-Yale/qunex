        ...
        if len(running) < cores:
            if calls:
                ...
                try:
                    ...
                    # header
                    print "# Generated by QuNex %s on %s" % (get_qunex_version(), datetime.now().strftime("%Y-%m-%d_%H.%M.%s"))
                    print "#"
...
def createSessionFile(command, sfolder, session, subject, overwrite, prefix=""):
    ...
    sout = open(sfile, 'w')
    print >> sout, "# Generated by QuNex %s on %s\n" % (get_qunex_version(), datetime.datetime.today())
    print >> sout, "#\n"
