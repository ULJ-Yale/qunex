            ...
            for field in ['dicom', 'raw_data', 'data', 'hpc']:
                if field in dic:
                    if not os.path.exists(dic[field]) and verbose:
                        print("WARNING: session %s - folder %s: %s specified in %s does not exist! Check your paths!" %
...
def get_sessions_list(listString, filter=None, sessionids=None, sessionsfolder=None, verbose=False):
    ...
    if filter is not None and filter.strip() != "":
        try:
            filters = [[f.strip() for f in e.split(':')]
                       for e in filter.split("|")]
        except:
            raise ge.CommandFailed("get_sessions_list", "Invalid filter parameter", "The provided filter parameter is invalid: '%s'" % (
                filter), "The parameter should be a '|' separated  string of <key>:<value> pairs!", "Please adjust the parameter!")
            ...
            if calls:
                ...
                try:
                    if 'shell' in call and call['shell']:
                        running.append({'call': call, 'sout': sout, 'p': subprocess.Popen(
                            call['args'], stdout=sout, stderr=sout, bufsize=0, shell=True)})
                    else:
                        running.append({'call': call, 'sout': sout, 'p': subprocess.Popen(
                            call['args'], stdout=sout, stderr=sout, bufsize=0)})

                    if call['sout']:
                        print(prepend + "started running %s at %s, track progress in %s" %
                              (call['name'], str(datetime.now()).split('.')[0], call['sout']))
                    else:
                        print(prepend + "started running %s at %s" %
                              (call['name'], str(datetime.now()).split('.')[0]))
                except:
                    print(
                        prepend + "ERROR: failed to start running %s. Please check your environment!" % (call['name']))
                    completed.append(
                        {'exit': -9, 'name': call['name'], 'log': call['sout'], 'args': call['args']})
                continue
        ...
        for n in range(len(running)):
            ...
            if running[n]['p'].poll() is not None:
                ...
                if running[n]['call']['sout']:
                    print(prepend + "finished running %s (exit code: %d), log in %s" %
                          (running[n]['call']['name'], running[n]['p'].poll(), running[n]['call']['sout']))
                else:
                    print(prepend + "finished running %s (exit code: %d)" %
                          (running[n]['call']['name'], running[n]['p'].poll()))
                completed.append({'exit': running[n]['p'].poll(
                ), 'name': running[n]['call']['name'], 'log': running[n]['call']['sout'], 'args': running[n]['call']['args']})
                done.append(n)
        if done:
            while done:
    ...
    with ProcessPoolExecutor(max_workers=cores) as executor:
        for call in calls:
            future = executor.submit(
                runWithLog, call['function'], call['args'], call['logfile'], call['name'], prepend)
...
def checkFiles(testFolder, specFile, fields=None, report=None, append=False):
    ...
    if report:
        if type(report) is types.FileType:
            ...
            try:
                ...
                raise ge.CommandFailed("checkFiles", "Report file could not be opened", "Failed to open a report file for writing: %s" % (
                    report), "Please check your settings and paths!")
    ...
    if not os.path.exists(testFolder):
        print("The folder to be tested does not exist: %s \nPlease check your settings and paths!" % (
            testFolder), file=rout)
        print("\n#-----------------=== End Full File Report ===----------------------", file=rout)
        if fileClose:
            rout.close()
        raise ge.CommandFailed("checkFiles", "Folder to test does not exist", "The folder to be tested does not exist: %s" % (
            testFolder), "Please check your settings and paths!")
    ...
    if not os.path.exists(specFile):
        ...
        if fileClose:
            rout.close()
        raise ge.CommandFailed("checkFiles", "Specification file does not exist", "The specification file to test folder against does not exist: %s" % (
            specFile), "Please check your settings and paths!")
    ...
    files = [[f.strip().split() for f in e.split('|')]
...
def getLogFile(folders=None, tags=None):
    ...
    if 'logfolder' not in folders:
        raise ge.CommandFailed("getLogFile", "Logfolder not found",
    ...
    logname = os.path.join(folders['logfolder'],
                           'comlogs', "tmp_%s.log" % (logname))
...
def closeLogFile(logfile=None, logname=None, status="done"):
    ...
    if logname:
        ...
        newname = re.sub('^(tmp_|done_|error_|incomplete_|)',
                         status + '_', newname)
...
def createSessionFile(command, sfolder, session, subject, overwrite, prefix=""):
    ...
    if os.path.exists(sfile):
        if overwrite == 'yes':
            ...
            raise ge.CommandFailed(command, "session.txt file already present!", "A session.txt file alredy exists [%s]" % (
