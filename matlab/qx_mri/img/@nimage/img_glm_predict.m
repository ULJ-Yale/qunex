...
if isempty(raw)
    ...
    if isfield(glm.glm, 'use')
        ...
        if sum(abs(predicted.use - glm.glm.use)) > 0
            fprintf('WARNING: When processing files, use information do not match! Using information provided in GLM. (glm file: %s, source file: %s)', predicted.filenamepath, glm.filenamepath);
        end
        predicted.use = glm.glm.use;
    else
