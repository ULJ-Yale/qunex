function [gbcmaps] = fc_compute_seedmaps(bolds, roiinfo, frames, targetf, options)
...
%
%               gbcmaps   
%                   the type of GBC maps to compute: mFz, aFz, pFz, nFz, aD, pD, nD, 
%                   mFzp, aFzp ... Multiple types and their parameters can be specified, 
%                   separated by ``|`` in a string: 
%    
...
%
%               saveind  
%                   Whether and how to save the resulting GBC maps. The options are:
%
%                   separate
%                       save each map in a sparate file. 
%                   by_extraction
%                       save all GBC maps generated from the same data extraction 
%                       in the same file, ordered as specified in the gbcmap option
%                   by_gbc_map
%                       save all GBC maps of the same type in the same file, ordered
%                       as specified by frames parameter
%                   joint
%                       save all GBC maps in the same file, ordered by extraction and 
%                       then by GBC map type
%                   none
...
%   =======
%
%   The function returns an image object with all the maps for all the timeseries
%   extractions in individual frames, ordered first by extraction and then by 
%   map type (all the maps for first extraction, then all the maps for the second
%   extraction ...) as specified by frames and gbcmaps, respectively.
%
%   If ``saveind`` is set to 'separate', each generated GBC map will be saved in 
%   a separate file, named: 
%
%   `<targetf>/<name>[_<subjectname>][_<title>]_gbc_<type of gbc>_<parameter value>`
%
%   If ``saveind` is set to 'by_extraction' all the maps for a single extraction
%   will be saved in separate files, named:
%
%   `<targetf>/<name>[_<subjectname>][_<title>]_gbc`
%
%   If ``saveind` is set to 'by_gbc_map' GBC maps for all the extractions will be 
%   saved in separate files named:
%
%   `<targetf>/<name>[_<subjectname>]_gbc_<type of gbc>_<parameter value>`
%
%   If ``saveind` is set to 'joint', all the GBC maps for all the extractions will be 
%   saved in a single file named:
%
%   `<targetf>/<name>[_<subjectname>]_gbc`
%
%   USE
%   ===
% 
%   The function computes the specified GBC maps for the specified timeseries 
%   extractions. If an event string is provided, it has to start with a path to the 
%   .fidl file to be used to extract the events, following by a pipe separated list 
%   of event extraction definitions::
%
...
%   
%   From the resulting timeseries images, all the specified GBC maps will be computed.
%
...
default = 'roimethod=mean|eventdata=all|ignore=use,fidl|badevents=use|gbcmaps=mFz:0|saveind=none|subjectname=|verbose=false|debug=false';
...
for n = 1:nsets
    ...
    gbcmaps(n).fc    = fc;
    gbcmaps(n).roi   = roi.roi.roinames;
    gbcmaps(n).N     = ts.frames;
...
if ~isempty(options.saveind)
    ...
    for n = 1:nsets
        if gbcmaps(n).title, settitle = ['_' gbcmaps(n).title]; else settitle = ''; end
        ...
        if any(ismember(options.saveind, {'p', 'z', 'jp', 'jz'}))
            Z = gbcmaps(n).fc;
            Z.data = fz.data/(1/sqrt(gbcmaps(n).N-3));
        end
        ...
        if verbose; fprintf('         ... saving set %s, roi:', gbcmaps(n).title); end
        ...
        if any(ismember(options.saveind, {'r', 'fz', 'z', 'p', 'cv'}));
            ...
            for r = 1:nroi
                ...
                if verbose; fprintf(' %s', gbcmaps(n).roi{r}); end

                % --- prepare basename

                basefilename = sprintf('seed_%s%s%s_%s', subjectname, name, settitle, gbcmaps(n).roi{r});
                ...
                for sn = 1:length(options.saveind)
                    switch options.saveind{sn}
                        case 'r'
                            t = gbcmaps(n).fc.sliceframes([1:nroi] == r);                                              
