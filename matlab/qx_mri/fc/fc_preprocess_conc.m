...
%                     
%					If empty, the defaults from img_compute_scrub are used.
%
...
%   ===
%
%   fc_preprocess_conc is a complex command initially used to prepare BOLD files
...
function [img coeff] = regressNuisance(img, omit, nuisance, rgss, rtype, ignore, options, Xroot, rmodel)
 ...
 %----- movement derivatives
    ...
    pre      = sprintf('%s# fidl: %s\n# model: %s\n# bolds: %d\n# effects: %s\n# effect: %s\n# eindex: %s\n# ignore: %s\n# event: %s\n# frame: %s', header, rmodel.fidl.fidl, rmodel.description, nbolds, xeffects, xeffect, xeindex, rmodel.ignore, xevents, xframes(1:end-1));
