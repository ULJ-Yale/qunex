            ...
            elseif ismember(model.regressor(m).hrf_type, {'boynton', 'spm', 'gamma'})
                ...
                if model.regressor(m).betaseries > 0
                    ...
                    for iev_global = 1:n_global_events
                        ...
                        % Convolve, normalize and downsample both with HRF in a unified helper
                        ts_selected = hrf_convolve(ts_selected, hrf, nframes, model.regressor(m).normalize);
                        ts_others   = hrf_convolve(ts_others  , hrf, nframes, model.regressor(m).normalize);
...
% -------------------------------------------------------------------------
% Unified HRF convolution, normalization, and downsampling helper
%
% Inputs:
%   ts_up     - upsampled event timeseries at 100 Hz (column or row vector)
%   hrf       - HRF kernel at 100 Hz
%   nframes   - number of BOLD frames in the run
%   normalize - 'uni' (divide by positive HRF AUC), 'run' (max=1 per run), or otherwise none
%
% Output:
%   ts_ds     - 1 x nframes row vector timecourse
function ts_ds = hrf_convolve(ts_up, hrf, nframes, normalize)

    % Ensure column vector for convolution
    ts_up = ts_up(:);

    % Convolve and crop back to 100*nframes
    ts = conv(ts_up, hrf);
    ts = ts(1:100*nframes);

    % Universal normalization by positive AUC of HRF (if requested)
    if strcmpi(normalize, 'uni')
        denom = sum(hrf(hrf > 0));
        if denom > 0
            ts = ts ./ denom;
        end
    ...
    ts_ds = mean(reshape(ts, 100, nframes), 1);

    % Per-run normalization to unit amplitude (if requested)
    if strcmpi(normalize, 'run')
        mx = max(ts_ds);
        if mx > 0
            ts_ds = ts_ds ./ mx;
        end
