function [] = fc_ComputeSeedMapsGroup(flist, roiinfo, frames, targetf, options)
...
%   files:
%
%       fc_ComputeSeedMaps('scz.list', 'CCNet.names', ...
...
[session, nsub, nfiles, listname] = g_ReadFileList(flist, options.verbose);
...
for n = 1:nsub
    ...
    if isfield(session(n), 'conc')
        ...
        reference_file = g_ReadConcFile(session(n).conc);
