...
for n = 1:length(tdata)
	...
	fprintf('\nOpenNewDocument("task %s","task partial correlations", "subject: %s", "data: %s, %s side")\n', [tdata(n).subject '-' tdata(n).data '-' side], tdata(n).subject, tdata(n).data, side);
	fc_DrawConnections(tdata(n).corr.pr, tdata(n).corr.p_pr, t, droi, cord);

	fprintf('\nOpenNewDocument("rest %s","rest partial correlations", "subject: %s", "data: %s, %s side")\n', [tdata(n).subject '-' rdata(n).data '-' side], rdata(n).subject, rdata(n).data, side);
	fc_DrawConnections(rdata(n).corr.pr, rdata(n).corr.p_pr, t, droi, cord);
	
	fprintf('\nOpenNewDocument("task-rest %s","task-rest partial correlations", "subject: %s", "data: %s, %s side")\n', [tdata(n).subject '-' tdata(n).data '-' side], tdata(n).subject, tdata(n).data, side);
	fc_DrawConnections(dif(n).mgdiffpr, dif(n).pgdiffpr, t, droi, cord);
