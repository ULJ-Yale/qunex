...
usage() {
    ...
    echo "-- DESCRIPTION:"
    echo ""
    echo " This function implements validation of DICOMs and BIDS datasets for "
    echo " uploads and downloads via several functions."
    echo ""
    ...
    echo ""
    echo " The function supports downloads via the FileLocker (http://filelocker2.sourceforge.net/) "
    echo " This allows downloads via the FileLocker tool onto a local file system."
    echo "   --> Note: If using filelocker to invoke this function you will need "
    echo "       a credential file in your home folder from filelocker_cli.conf " 
    echo ""
    ...
    echo "   --runtype=<specify_run_type>                      Specify from available --runtype options: "
    echo ""
    echo "                                                     --> daxupload          ... Runs upload to XNAT via DAX functions"
    echo "                                                     --> daxdownload        ... Runs download from XNAT via DAX functions"
    echo "                                                     --> filelockerdownload ... Runs download from FileLocker"
    echo "                                                     --> datavalidate       ... Runs data validation for downloaded file"
    echo ""
