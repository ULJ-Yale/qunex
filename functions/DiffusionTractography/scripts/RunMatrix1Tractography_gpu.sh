...
#Probtrackx Options -- removed --omatrix4 out of GPU version
Options="--forcedir --meshspace=caret --nsteps=2000 --fibthresh=0.05 --loopcheck --randfib=2 --forcefirststep --opd --omatrix1 --sampvox=${DiffusionResolution} --verbose=1"
...
if [ ! $HemisphereSTRING = "Whole" ] ; then
  for Hemisphere in $HemisphereSTRING ; do
	...
	module load Apps/FSL/5.0.6
	module load GPU/Cuda/5.0
	module load GPU/Cuda/6.5
	module load GPU/Cuda/7.0
	source /usr/local/cluster/hpc/Apps/FSL/5.0.6/fsl/etc/fslconf/fsl.sh

    echo "probtrackx2_gpu ${probtrackx_args}"
    time probtrackx2_gpu ${probtrackx_args}
